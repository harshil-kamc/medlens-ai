  ...
  return (
    <div className={`rounded-xl border p-4 ${dark ? "border-slate-800 bg-slate-900/40" : "border-slate-200 bg-white"}`}>
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className={`text-sm font-semibold ${dark ? "text-slate-200" : "text-slate-800"}`}>Preset Case Selector</h3>
      </div>
      <div className="grid gap-2 sm:grid-cols-2">
        {PRESET_CASES.map((c) => {
          ...
          return (
            ...
            >
              <p className={`text-xs font-semibold ${active ? "text-brand-200" : dark ? "text-slate-300" : "text-slate-700"}`}>{c.label}</p>
